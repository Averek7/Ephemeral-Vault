    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Deposit to Vault" icon="📥">
      <div className="space-y-4">
        <Input
          ...
          error={amount && !withinBalance ? 'Insufficient wallet balance' : amount && !withinLimit ? 'Exceeds approved limit' : undefined}
        />

        {amountNum > 0 && (
          <div className="rounded-lg bg-vault-bg border border-vault-border p-4 space-y-2 text-sm">
            <InfoRow label="Current Vault" value={`${formatSOL(vault.currentBalance)} SOL`} />
            <InfoRow label="Depositing" value={`+ ${formatSOL(amountNum)} SOL`} color="green" />
            <div className="border-t border-vault-border pt-2">
              <InfoRow label="After Deposit" value={`${formatSOL(afterDeposit)} SOL`} bold />
            </div>
            <InfoRow label="Approved Limit" value={`${formatSOL(vault.approvedAmount)} SOL`} />
            <InfoRow label="Remaining" value={`${formatSOL(Math.max(0, remainingApproved - amountNum))} SOL`} />
          </div>
        )}
...
function InfoRow({ label, value, color, bold }: { label: string; value: string; color?: 'green'; bold?: boolean }) {
  return (
    <div className="flex justify-between">
      <span className="text-vault-muted">{label}</span>
      <span className={`mono text-sm ${color === 'green' ? 'text-sol-green' : 'text-white'} ${bold ? 'font-bold' : ''}`}>{value}</span>
    </div>
  );
}
